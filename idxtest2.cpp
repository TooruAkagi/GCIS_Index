...
int psread(int p,bool *ar,int D2start,int D2end,int basekey){
    ...
    int chr;
    if(DEBUGFLAG==1){
        if(p>=dc2s[1]){printf("<%d>",basekey);}
        else{
            //printf("%c",basekey);
            chr = fgetc(grobalfp);
            if(chr!=basekey){printf("[%d != %d]",chr,basekey);exit(1);}
        }
        //chr = fgetc(grobalfp);printf("%c",chr);
    }
    int a;
    int turnsl = 0;
    int st = D2start;
...
void uhdisp(int tgm){
        int D2start = getd2(tgm); //開始１
        int D2end = getd2(tgm+1); //end位置
        int tgm3 = gethead(tgm); //分割のヘッドナンバーを読む
        psread(tgm,k,D2start,D2end,tgm3); //tgm版の分割を読む
}

void uhdispfull(int tgm,int fround){
        //printf("[%d]",tgm);
        int D2start = getd2(tgm); //開始１
        int D2end = getd2(tgm+1); //end位置
        int tgm3 = gethead(tgm); //分割のヘッドナンバーを読む
        if(tgm<dc2s[1]){psread(tgm,k,D2start,D2end,tgm3);}
        else{
            uhdispfull(tgm3+dc2s[fround-1],fround-1);
            int a;
            int turnsl = 0;
            int st = D2start;
            for(int i = 0;i<1000;i++){
...
int ptrans(int *B1,int *B2,int *B3,int *Ls,int *Rs,int startm,int *m,int mrlen,int round){
    ...
    //printf("[");for(int y8=0;y8<mrlen;y8++){printf("%d ",m[startm+y8]);}printf("]");
        ...
        if(m[i-1] > m[i]){
            if(tn==0){ //m[i-1] & m[i] == L & S
                if(B2[round] == 0){B2[round] = len;len = 0;flag++;}
                    ...
                    Rpptr-=(len-1);
                    //printf("\n分割[");
                    //for(int y8=0;y8<len;y8++){printf("%d ",m[i+y8]);}
                    //printf("]を探す\n");
...
int patternmatching1(int *m,int absm,int fround){
    ...
    //int fi= 0;
    int msta = 0;
    int mend = 0; //e3[msta -- (mend-1)]までもじが入っている
    //exit(8);
    while(1){ 
        if(i<Sizelist[0]){
            nem[fround-1] = NS[i];
            ...
            else{mend++;if(mend>=upc){mend=0;}} //入れたらmendをひとつ進める
        }
        else{
        ...
        if(e3[1]>=0){
            if(round>0){len+= D3access(e3[1]);}
            else{len++;}
        }
        if(msta==mend){
            mk = ckcore(e3,m,startm,mrlen,kmpc,msta,upc);
            if(mk==1){
                core++; //コア発見
            //if(len>5000000){exit(2);}
...
int bitload(bool *ar,unsigned char a,int p,int mx){
...
int main(){
    ...
    msize = queryload(m);
    for(int uj=0;uj<loops;uj++){
        printf("\ndc2s[%d] = %d",uj,dc2s[uj]);
    }
    for(int uj=0;uj<Sizelist[0];uj++){
        uhdispfull(NS[uj]+dc2s[loops-1],loops-1);
    }
    printf("\n完全一致");
    exit(1);
    for(int i = 0;i<1000;i++){
